...
class projects
{
  ...
  function __construct()
  {
    ...
    if (!empty($this->username) or ($this->username == 'jimmie')) {
      ...
      printjson(array('success' => 0, 'error_log' => 'missing username ' . $_POST['username']));
  ...
  public function update_stats() {
    ...
    public function calm_settlement($project_id) {
      $qry_string = "SELECT *
        FROM projects
        WHERE
        (
        ...
        if($query->num_rows >= 2) {
          ...
          // should be granted to all pets
          mysqli_query($this->conn, "UPDATE project_attacks SET attacks_claw = 1 WHERE attacks_project_id = $project_id");
        }
    }
    ...
    public function execute_projects(){
                ...
                if (strtotime($this->now) >= strtotime($battle_until_date)) {
                  ...
                  if (mysqli_query($this->conn,
                    ...
                    if ($battle_winner_project_id == (int)$project_id) {
                      ...
                      $this->earn_achievement($this->username, $achievement_id, 'battles');
                      $battle_awards = $this->give_rewards($project_id, $battle_winner_reward_pet);

                      // earn experience as well.
                      require('../users/gain_experience.php');
                      $experience = new experience($conn);

                      $exp = $experience->gain_experience($this->username, 1000);
        ...
        if ($timespan >= $life_span) {
          ...
          if ($timespan >= $next_stage) {
            ...
            if ($stage == 2) {
              ...
              $this->calm_settlement($project_id);
      ...
      // execute projects
      printjson(array('pets' =>  $this->rows, 'achievements' => $this->achievements_container), true);
